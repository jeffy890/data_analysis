...
def get_data(base_df, fn_to_s, scripts_option):
    ...
    list_titles = [
        ...
        "degree ja", "degree en", 
        "degree date", 
        "degree institution ja", 
        "degree institution en", 
        "rsmp id", 
        "erad", 
        "orcid", 
        "jglobal", 
    ...
    for index, row in base_df.iterrows():
        ...
        if dt.check_json(search_id) == 1:
            templist.extend([1, search_id])
            templist.extend(dt.json_load(search_id))
            ...
            try:
                ...
                templist.extend(dt.get_json(base_url_api, search_id))
                print("got data")
            ...
            # if there's no id result, then name search starts
                ...
                try:
                    ...
                    if scripts_option == 1:
                        templist.extend(dt.find_the_one(search_id, serach_name, all_links))
                    else:
                        templist_multiple = (dt.get_jsons(search_id, serach_name, all_links))
                    print("name hit!")
